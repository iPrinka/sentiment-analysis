...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # Your code here
    zarr = (feature_matrix@theta + theta_0)*labels
    loss = 0
    for z in zarr:
        if z < 1: 
            loss = loss + 1-z
...
def perceptron_single_step_update(
    ...
    if label*(feature_vector@theta + theta_0) < 0.001:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            iter = iter + 1
    theta = (1/iter) * sum_theta
    theta_0 = (1/iter) * sum_theta_0
    return (theta, theta_0)
...
def classifier_accuracy(
    ...
    train_y_hat = classify(train_feature_matrix, theta, theta_0)
    val_y_hat = classify(val_feature_matrix, theta, theta_0)
    train_accuracy = accuracy(train_y_hat, train_labels)
    val_accuracy = accuracy(val_y_hat, val_labels)
